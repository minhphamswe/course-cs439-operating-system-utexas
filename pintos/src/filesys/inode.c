    ...
    while (bytes_left > 0)
    {
      ...
      { // Successfully allocated disk space for subsequent INODE
        ...
        if (prev_node != NULL && prev_node != current_node) {
          ASSERT(prev_node->next != prev_node);
          ASSERT(prev_node->sector != ptr_get_address(&prev_node->data.doubleptr));
